param([Alias('Version')][string]$AppVersion='0.1.1')
...
try {
 ...
 & "$vc/bin/Hostx64/x64/cl.exe" /nologo /MT /EHsc /std:c++17 /DUNICODE /D_UNICODE /utf-8 /W4 /O2 "/I$vc/include" "/I$sdk/Include/$sdkVersion/ucrt" "/I$sdk/Include/$sdkVersion/shared" "/I$sdk/Include/$sdkVersion/um" "/I$sdk/Include/$sdkVersion/winrt" "/I$sdk/Include/$sdkVersion/cppwinrt" "/I$webview/build/native/include" "/Fo$out/" "$PSScriptRoot/native/Host.cpp" "$PSScriptRoot/native/GlassRenderer.cpp" "$out/Delo.res" /link /SUBSYSTEM:WINDOWS /MANIFEST:EMBED "/MANIFESTINPUT:$PSScriptRoot/native/app.manifest" "/LIBPATH:$vc/lib/x64" "/LIBPATH:$sdk/Lib/$sdkVersion/um/x64" "/LIBPATH:$sdk/Lib/$sdkVersion/ucrt/x64" "$webview/build/native/x64/WebView2LoaderStatic.lib" WindowsApp.lib User32.lib Gdi32.lib Dwmapi.lib D3D11.lib DXGI.lib D3DCompiler.lib Dcomp.lib Wtsapi32.lib Shell32.lib Ole32.lib Shlwapi.lib Advapi32.lib Version.lib "/OUT:$out/Delo.exe"
 if($LASTEXITCODE -ne 0){throw 'Delo compilation failed'}
 foreach($folder in @('ui','core')){Copy-Item -LiteralPath "$PSScriptRoot/$folder" -Destination $out -Recurse -Force}
 Copy-Item "$PSScriptRoot/native/Glass.hlsl" $out
