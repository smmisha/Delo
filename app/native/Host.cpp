...
std::wstring CurrentLanguage(){return store&&store->State()?std::wstring(store->State().GetNamedObject(L"settings",JsonObject{}).GetNamedString(L"language",L"ru")):std::wstring(L"ru");}
// The tray icon is the one place demo mode is announced, so its tooltip names the mode.
std::wstring TrayTip(){return demoMode?L"Delo · "+std::wstring(TrayLabel(CurrentLanguage(),5)):std::wstring(L"Delo");}
...
void BeginScreenshot(){
    if(demoMode)return; // the widgets are already visible to capture
...
// Presenting. Viewers see whatever the capture API is given, and the widget hides from it
// by design: its glass is built from a capture of the desktop behind it, so it must not
// appear in that capture itself. Demo mode gives the glass up. The renderer is switched off
// first (no device, no capture session, nothing left to compose stale), then the windows are
// released to capture, and the interface falls back to its matte material. Turning it off
// hides the windows again before the renderer starts capturing, so it never sees itself.
