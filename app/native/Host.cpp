...
// Synchronise the final geometry and material after Windows finishes a gesture.
...
void ShowPassive(View& v){v.visible=true;ShowWindow(v.hwnd,SW_SHOWNOACTIVATE);if(v.controller)v.controller->put_IsVisible(TRUE);JsonObject p;p.Insert(L"visible",Bool(true));Event(v,L"visibility",p);}
...
LRESULT CALLBACK WindowProc(HWND hwnd,UINT message,WPARAM w,LPARAM l){
    ...
    if(v){
        ...
        if(message==delo::GlassFrameReadyMessage){if(v->visible&&v->glass&&!v->captureFrozen)v->glass->Tick();return 0;}
        ...
        if(message==WM_SIZE||message==WM_MOVE){
            ...
            if(v->gesture&&v->glass&&!v->captureFrozen)v->glass->Tick();
            return 0;
        }
    ...
    if(hwnd==control){
        ...
        if(message==WM_TIMER&&w==6){KillTimer(control,6);++traySingleClicks;Quick();return 0;}
    }
