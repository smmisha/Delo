...
try{
 ...
 await main.host('pin',{pinned:true});await main.host('show');
 await main.evaluate(`(()=>{document.querySelectorAll('dialog[open]').forEach(d=>d.close());document.querySelector('#task-input').value='Audit completed task';document.querySelector('#task-input').dispatchEvent(new Event('input',{bubbles:true}));document.querySelector('#entry').requestSubmit();return true;})()`);await wait(300);
 await main.evaluate(`(()=>{const row=[...document.querySelectorAll('.task')].find(r=>r.querySelector('.task-title').textContent==='Audit completed task');row.querySelector('.complete').click();return true;})()`);await wait(300);
 ...
 await main.click('#settings-open');await main.click('#open-settings');await main.evaluate(`document.querySelector('.settings-fields').scrollTop=0`);await screenshot(main,path.join(output,'settings-light.png'));
