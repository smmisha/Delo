(async()=>{
  ...
  for(const title of titles){input.value=title;input.dispatchEvent(new Event('input',{bubbles:true}));document.querySelector('#entry').requestSubmit();const end=Date.now()+5000;while(Date.now()<end){const value=await host.request('load');if(input.value===''&&value.state?.tasks.some(task=>task.title===title))break;await wait(25);}}
