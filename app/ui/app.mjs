...
function closeTooltip(){if(tipTimer)clearTimeout(tipTimer);tipTimer=null;if(tipNode){tipNode.classList.remove('tip-open');tipNode=null;}}
document.addEventListener('pointerover',event=>{
  ...
  tipNode=node;
  tipTimer=setTimeout(()=>{tipNode?.classList.add('tip-open');},1000);
});
...
$('#open-archive').addEventListener('click',()=>{closeAppMenu();openCollection('archive');});
$('#open-trash').addEventListener('click',()=>{closeAppMenu();openCollection('trash');});
$('#open-settings').addEventListener('click',()=>{closeAppMenu();openSettings();});
...
host.addEventListener('nativeChanged',event=>{native={...event.detail};render();if(native.hotkeyError)report(Error(native.hotkeyError));});host.addEventListener('visibility',event=>{nativeVisible=event.detail.visible;lastUndo=mono();renderUndo();if(nativeVisible&&quick)$('#task-input').focus();});host.addEventListener('focusQuick',()=>$('#task-input').focus());
