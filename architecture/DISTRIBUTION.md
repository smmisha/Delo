...
Локальный пакет 0.1.6 собран после исправления подсказок действий задач при закреплённом заголовке группы. Установщик `Delo-0.1.6-windows-x64-setup.exe` имеет размер `185564809` байт и SHA-256 `44EAF96739A36D178C0BD6A87E113830CD517BAF92058590F0473CBFAA422FD2`; `Delo.exe` сообщает ProductVersion `0.1.6`. Пользовательские данные до упаковки и после перезапуска совпали побайтно. Пакет остаётся неподписанным.
