...
Исправляющий установщик `0.1.5` возвращает незакреплённый список за активное обычное окно после открытия из трея. Размер — `185568458` байт, SHA-256 — `054CD96FC15A1343C715BC8A1E1A50F50F97B2CFCF5D507EF94F163126F99C80`. Отдельная установка дала версию `0.1.5`, установленный EXE совпал с release-сборкой, оконный harness прошёл 12/12, удаление завершилось кодом 0. Точный production-сценарий с текущим окном Codex подтвердил: чужое окно стало foreground, Delo получила desktop owner, потеряла topmost и оказалась под Codex.
...
Публичный репозиторий и release `v0.1.4` опубликованы. GitHub подтвердил размер и SHA-256 установщика; повторный аудит отслеживаемых файлов нашёл 0 секретных шаблонов, 0 личных абсолютных путей и 0 файлов `app/dist` в Git. Для полного V12 остаётся сопоставление с чистой системой без WebView2.
